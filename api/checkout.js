...
function cardExclusions() {
  // Checkout Pro não permite excluir account_money.
  return ['bank_transfer', 'ticket', 'atm', 'debit_card', 'prepaid_card'];
}
...
function pixPayload(payment, id, amount, seats, expiresAt, reused = false) {
  ...
  return {
    ...
    reused,
    pix: { qrCode: tx.qr_code || '', qrCodeBase64: tx.qr_code_base64 || '', ticketUrl: tx.ticket_url || '' }
  };
}
...
async function postMp(path, body, idempotencyKey) {
  ...
  const response = await fetch(`https://api.mercadopago.com${path}`, { method: 'POST', headers, body: JSON.stringify(body) });
...
async function checkCapacity(tripId, reservationId, capacity) {
  for (const wait of [300, 800, 1600, 2500]) {
    ...
    try {
      ...
      console.warn('Capacity recheck failed', e?.message || e);
    }
  }
  return null;
}
  ...
  try {
    ...
    if (method === 'pix') {
      ...
      if (existing?.id && ['pending', 'approved'].includes(existing.status)) {
        ...
        if (existing.status === 'approved' || tx.qr_code || tx.ticket_url) {
          return json(res, 200, pixPayload(existing, id, Number(existing.metadata?.amount || total), Number(existing.metadata?.seats || seats), existing.date_of_expiration, true));
        }
      }
    } else {
      ...
      if (existing?.id && existing?.init_point) {
        ...
        if (active) return json(res, 200, { reservationId: id, preferenceId: existing.id, paymentMode: 'checkout_pro', checkoutUrl: existing.init_point, amount: Number(existing.metadata?.amount || total), seats: Number(existing.metadata?.seats || seats), expiresAt: existing.expiration_date_to, reused: true });
      }
    }

    let used;
    try { used = await usedSeatsForTrip(trip.id); }
    catch { return json(res, 503, { error: 'Não foi possível confirmar as vagas agora. Tente novamente em alguns segundos.' }); }
    if (used + seats > trip.capacity) return json(res, 409, { error: 'Não há vagas suficientes disponíveis.' });

    const now = new Date();
    const expiresAt = new Date(now.getTime() + 30 * 60 * 1000);
    const metadata = {
      reservation_id: id, request_id: requestId || id, trip_id: trip.id, trip_title: trip.title,
      trip_date: trip.date, location: trip.location, variant_id: variant.id, variant_name: variant.name,
      payment_choice: method, quantity, seats_per_unit: seatsPerUnit, seats, unit_price: Number(unitPrice), amount: total
    };
    ...
    if (method === 'pix') {
      ...
      const paymentBody = {
        ...
        payer: { email: customer.email, first_name: names.shift() || customer.name, last_name: names.join(' '), identification: { type: 'CPF', number: customer.cpf } },
        metadata
      };
      ...
      if (fits !== true) {
        ...
        return json(res, fits === false ? 409 : 503, { error: fits === false ? 'As últimas vagas foram ocupadas. O PIX foi cancelado.' : 'Não foi possível validar a reserva. O PIX foi cancelado por segurança.' });
      }
    ...
    const preferenceBody = {
      items: [{ id: `${trip.id}-${variant.id}`, title: `${trip.title} - ${variant.name}`, description: `${trip.date} • ${trip.location}`, category_id: 'tourism', currency_id: 'BRL', quantity, unit_price: Number(unitPrice) }],
      payer: { name: names.shift() || customer.name, surname: names.join(' '), email: customer.email, phone: splitPhone(customer.phone), identification: { type: 'CPF', number: customer.cpf } },
      external_reference: id,
      statement_descriptor: 'TRILHEIROS',
      notification_url: `${baseUrl}/api/webhook`,
      back_urls: { success: `${baseUrl}/?retorno=sucesso&reserva=${encodeURIComponent(id)}`, pending: `${baseUrl}/?retorno=pendente&reserva=${encodeURIComponent(id)}`, failure: `${baseUrl}/?retorno=falha&reserva=${encodeURIComponent(id)}` },
      auto_return: 'approved',
      expires: true,
      expiration_date_from: now.toISOString(),
      expiration_date_to: expiresAt.toISOString(),
      payment_methods: { excluded_payment_types: cardExclusions().map(id => ({ id })), installments: 12 },
      metadata
    };
    ...
    if (fits !== true) {
      ...
      return json(res, fits === false ? 409 : 503, { error: fits === false ? 'As últimas vagas foram ocupadas. O checkout foi encerrado.' : 'Não foi possível validar a reserva. Tente novamente.' });
    }

    return json(res, 201, { reservationId: id, preferenceId: preference.id, paymentMode: 'checkout_pro', checkoutUrl: preference.init_point, amount: total, seats, expiresAt: expiresAt.toISOString() });
  } catch (error) {
    console.error(error);
